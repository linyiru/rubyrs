...
| 2 | **JSON** (parse/generate) | every API needs it; `as_json` | pure-Ruby canon (Rule 6) + `_json_native` accel | **pure canon shipped (2026-06)** via `src/stdlib_vendor/json.rb` — parse/generate/pretty_generate/dump/load, `to_json` + `as_json` mixins on basic types, `JSON::State`, `JSON::JSONError`/`ParserError`/`GeneratorError`/`NestingError` hierarchy, `symbolize_names:` / `allow_nan:` / `max_nesting:` options; deterministic subset byte-identical to CRuby stdlib. `_json_native` Rust accelerator still pending. |
| 3 | **ActiveSupport-lite core-ext slice** (`blank?`, `present?`, `Hash#deep_*`, `String#camelize`) | the connective tissue every Ruby web app assumes | **`_pure_activesupport_lite` (Tier-3 pure-Ruby canon)** vendored | ~3–5 days achievable on current Tier-1 |
| 4 | **A data layer on `_sqlite`** (a Sequel-lite / ROM-lite query DSL, *not* ActiveRecord) | "Sinatra + a DB" is the first real app | `_sqlite` battery (Tier 3) + pure-Ruby DSL | battery designed, not built; harness v2 (M27) prerequisite for stateful parity |
