...
| 4 | **A data layer on `_sqlite`** (a Sequel-lite / ROM-lite query DSL, *not* ActiveRecord) | "Sinatra + a DB" is the first real app | `_sqlite` battery (Tier 3) + pure-Ruby DSL | **Phase 1+2 shipped (2026-06)**: `poc/sqlite/FINDINGS.md` discovery output + `docs/adr/0027-battery-sqlite.md` locks rusqlite + bundled libsqlite3 + single-conn `SQLite3::Database` + block-form transactions + LRU(100) prepared-statement cache + 7-class exception hierarchy + `Config::sqlite_allow_paths` sandbox + `cli-defaults`/`everything` Cargo aggregates. Phase 3+ (battery PoC → diff_framework S4 hooks → Sequel-lite DSL → parity fixture) pending. |
