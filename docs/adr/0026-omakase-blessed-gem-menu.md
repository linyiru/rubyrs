...
| 3 | **ActiveSupport-lite core-ext slice** (`blank?`, `present?`, `Hash#deep_*`, `String#camelize`) | the connective tissue every Ruby web app assumes | **`_pure_activesupport_lite` (Tier-3 pure-Ruby canon)** vendored | **Tier A+B+C+D-narrow shipped (2026-06)** via `src/stdlib_vendor/active_support_lite.rb` — `blank?`/`present?`/`presence` family on Object/NilClass/TrueClass/FalseClass/String/Array/Hash/Numeric, `Array#second/third/fourth/fifth/in_groups_of`, `Object#try/#in?`, Hash `symbolize_keys`/`stringify_keys` + `deep_*` + `deep_merge`, String `squish/camelize/underscore/dasherize/titleize/humanize/truncate/blank?`, **Numeric `#seconds/#minutes/#hours/#days/#weeks/#fortnights` + `ActiveSupport::Duration` value type (parts-preserving inspect, +/-/* arithmetic, Time integration, `#ago`/`#since`/`#from_now`/`#until`)**. Deterministic subset byte-identical to `active_support/all` on CRuby 8.x; pinned by `as_lite_canon` framework-parity fixture + new `tests/diff/activesupport_duration.rb` gem-oracle. **Tier D-wide deferred** (`#months`/`#years` calendar advance + `Time.current` + `Time.zone` — chain through `Time#advance` + tzinfo, see `poc/as_lite/GAPS.md` §Tier D for the D-narrow vs D-wide split). |
