...
| 2 | **JSON** (parse/generate) | every API needs it; `as_json` | pure-Ruby canon (Rule 6) + `_json_native` accel | **pure canon shipped (2026-06)** via `src/stdlib_vendor/json.rb`; `_json_native` accel still pending. Deterministic subset (Null/Bool/Integer/Float/String/Array/Hash) parses + generates byte-identical to CRuby stdlib; `to_json` mixin / `pretty_generate` / option kwargs deferred to follow-ups. |
