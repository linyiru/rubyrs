...
| 2 | **JSON** (parse/generate) | every API needs it; `as_json` | pure-Ruby canon (Rule 6) + `_json_native` accel | **pure canon shipped (2026-06)** via `src/stdlib_vendor/json.rb` — parse/generate/pretty_generate/dump/load, `to_json` mixin on basic types, `symbolize_names:` / `allow_nan:` options; deterministic subset byte-identical to CRuby stdlib. `_json_native` Rust accelerator still pending. |
