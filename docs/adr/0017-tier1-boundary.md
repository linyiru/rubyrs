...
| `$$` global read the host process's PID via `std::process::id()` | (was) `crates/rubyrs/src/vm/step.rs:365` (the `LoadGlobal("$$")` arm) | Rule 1 | **Closed (PR #88)**: `Config::pid: Option<i64>` injects an explicit PID. `None` (default) → `$$` returns `0` sentinel; `Some(n)` → returns `n`. CLI binary fills from `std::process::id()`. |
