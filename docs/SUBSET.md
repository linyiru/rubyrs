...
| Real nested-module namespacing (`Foo::Bar` after `module Foo; module Bar; end; end`) | Tier 2 | Lexical dual-write: constants/classes defined inside a module/class are now stored under BOTH the bare name AND the prefixed `Foo::Bar` key, so `Foo::Bar` resolves from outside AND bare `Bar` resolves from inside the scope. Doesn't model CRuby's full cref-walk lookup or `Module#nesting`, so two modules defining the same bare name collide at top-level (last writer wins). |
