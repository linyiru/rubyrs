...
// A user key.hash raising during Marshal.load must surface the USER's
...
// family): join-landing walk cancellation for every walk origin and
// the while-loop next supersede. ORACLE FLOOR >= 3.4.2: CRuby
// 3.4.0/3.4.1-prism had a bug window here ([Bug #21001], fixed in
// 3.4.2) and diverges on ~15 shapes — see the fixture header. The
// one deliberate rubyrs divergence (walk survives a block-`next`,
...
// class_variable_set create-path gen bump, toplevel fallback), super
// site caches (hot-site redefinition, define_method dual runtime
// names at one op site, sibling receivers + prepend-mid-stream,
// splat/kw ApplySuper shapes), and the Hash merge/slice/except
...
#[test] fn hash_literal_user_hash_eql() { run_diff("hash_literal_user_hash_eql"); }
...
#[test] fn autoload_own_scope_beats_superclass() { run_diff("autoload_own_scope_beats_superclass"); }
#[test] fn const_defined_invalid_uppercase_name() { run_diff("const_defined_invalid_uppercase_name"); }
#[test] fn module_class_freeze() { run_diff("module_class_freeze"); }
...
// Char->byte index / UTF-8-validity caches on StrCell: char-indexed
// slicing + match-at-pos on LARGE (cache-threshold+) strings, and the
// invalidate-on-mutation contract (<< / []= / sub! then re-slice).
#[test] fn string_char_index_cache() { run_diff("string_char_index_cache"); }
