...
module MultiJson
  ...
  # Real `multi_json` exposes `.encode` and `.generate` alongside
  # `.dump` (the older `.encode` is deprecated in newer gem
  # versions but still works). sinatra/json's
  # `resolve_encoder_action` probes the configured encoder for
  # `:encode` or `:generate` and falls back with
  # `"MultiJson does not respond to #generate nor #encode"`
  # otherwise — so the shim has to expose at least one. Provide
  # both to keep parity with the real gem's surface.
  def encode(obj, opts = {})
    dump(obj, opts)
  end
