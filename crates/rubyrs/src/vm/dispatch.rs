    ...
    pub(crate) fn do_call(&mut self, name_id: SymId, argc: usize, no_recv: bool, cache_id: u16) -> Result<(), Trap> {
        ...
        if let Value::Class(cls) = &recv {
            match (&*name, args.as_slice()) {
                ...
                // `Module#singleton_class` / `Object#singleton_class`
                // — CRuby returns the per-object metaclass (eigen-
                // class) that holds the receiver's singleton
                // methods. Real metaclass has its own identity,
                // its `instance_methods` are the original's
                ...
                // is true because both calls return the same
                // \`X\`), which is the property real consumers
                // (e.g. tilt's `@_init = self.class.singleton_class`
                // cache-invariant check at MRI lib/erb/compiler.rb:828
                // / 900) actually need. Methods called on the
                // \"singleton_class\" result are dispatched against
                // the receiver — for the Class arm here that means
                // singleton_methods are NOT visible as
                // instance_methods of the singleton-class object,
                // which diverges from CRuby. Documented in
                // SUBSET.md; revisit when a consumer relies on
                // the real metaclass shape.
                ("singleton_class", []) => {
