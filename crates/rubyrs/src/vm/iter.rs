    ...
    pub(crate) fn collection_call_block(&mut self, recv: &Value, name: &str, args: &[Value], block: ObjId, bypass_override: bool) -> Result<Option<Value>, Trap> {
        ...
        Ok(match (recv, name, args) {
            ...
            (Value::Array(id), "map", []) | (Value::Array(id), "collect", []) => {
                ...
                // Float-element / Int-result variant (ADR 0034 layer 3e): a Float
                // array mapped to an Integer array via a per-element Float->Int
                // conversion (`floats.map { x.round }`). All loops above decline.
                #[cfg(feature = "jit-native")]
                if let Some(out) = g.vm.try_native_floatint_map_loop(block, *id) {
                    return Ok(Some(Value::Array(out)));
                }
                let snapshot: Vec<Value> = g.vm.heap.array(*id).clone();
                g.vm.maybe_gc();
            ...
            (Value::Array(id), "group_by", []) => {
                ...
                // Float-element / Float-key variant (ADR 0034 layer 3e): a Float array
                // bucketed by a Float key (`floats.group_by { x * 2.0 }`). The Int-key
                // loops above decline (Int reader / Int-result block reject the Float key).
                #[cfg(feature = "jit-native")]
                if let Some(h) = g.vm.try_native_floatkey_groupby_loop(block, *id) {
                    return Ok(Some(h));
                }
                let snapshot: Vec<Value> = g.vm.heap.array(*id).clone();
                // Defensive pin of every heap-slot element. Without
