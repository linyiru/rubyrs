...
pub(crate) struct CapturedGraph {
    ...
    pub(crate) cvar_counter: u32,
    pub(crate) super_counter: u32,
...
pub(crate) struct VmImage {
    ...
    pub(crate) ivar_counter: u32,
    /// `vm.cache_counter.cvar` / `.sup` — size the cvar-owner and
    /// super-site cache vectors (campaign P4).
    pub(crate) cvar_counter: u32,
    pub(crate) super_counter: u32,
...
pub(crate) fn to_bytes(
    ...
    let img = VmImageRef {
        ...
        cvar_counter: graph.cvar_counter,
        super_counter: graph.super_counter,
...
const FORMAT_VERSION: u32 = 5; // bumped: cvar/super site cids in Op + their CidGen counters (campaign P4)
...
pub(crate) fn restore(vm: &mut crate::vm::Vm, img: VmImage) {
    ...
    vm.cache_counter = crate::compiler::CidGen {
        call: img.cache_counter,
        ivar: img.ivar_counter,
        cvar: img.cvar_counter,
        sup: img.super_counter,
    };
    vm.ensure_call_caches(img.cache_counter as usize);
    vm.ensure_ivar_caches(img.ivar_counter as usize);
    vm.ensure_cvar_caches(img.cvar_counter as usize);
    vm.ensure_super_caches(img.super_counter as usize);
    // Restore rewrites `class_vars` tables wholesale below —
    // stale any pre-restore cvar-owner cache entries (P4).
    vm.cvar_gen = vm.cvar_gen.wrapping_add(1);
