...
/// 1-based source line `raw`, shifted into the caller's coordinate
/// system by a proto's `line_base` (the line its first source line maps
/// to). `line_base == 1` is the identity. Used for backtraces so
/// `eval(src, b, file, line)` and `class_eval(src, file, line)` report
/// the line the caller asked for. Clamps to ≥ 1 (CRuby never reports a
/// non-positive line in a backtrace even when the offset arithmetic
/// would go ≤ 0).
pub(crate) fn line_with_base(raw: u32, line_base: i32) -> u32 {
    (raw as i64 + line_base as i64 - 1).max(1) as u32
}
