...
pub(crate) fn tr(node: &Node<'_>) -> SExpr {
    ...
    // `alias new old` keyword form — desugar into a synthetic
    // `alias_method :new, :old` Call so the existing compiler
    // intercept (compiler.rs, `Op::AliasMethod`) handles it.
    // Only the all-Symbol form is supported (the keyword's
    // syntactic shape — `alias prefer register` parses as
    // SymbolNode + SymbolNode in Prism). `alias $new $old`
    // (GlobalVariableNode operands) falls through unsupported.
    //
    ...
    // attr_*" SyntaxError.
    // Non-Symbol operand forms (alias $g1 $g2, etc.) — the
    // chain's `let (Some(...), Some(...)) = ...` fails for them
    // and the whole arm falls through to the unsupported-node
    // trail below, surfacing as AST_ERRORS.
