...
fn main() {
    ...
    // ---- register the DSL surface ----
    {
        ...
        rt.register_fn("tap", move |args| {
            if let [Value::Str(s)] = args {
                b.borrow_mut().taps.push(s.to_string_lossy());
            }
            Ok(Value::Nil)
        });
    }
    {
        ...
        rt.register_fn("brew", move |args| {
            if let [Value::Str(s)] = args {
                b.borrow_mut().formulae.push(s.to_string_lossy());
            }
            Ok(Value::Nil)
        });
    }
    {
        ...
        rt.register_fn("cask", move |args| {
            if let [Value::Str(s)] = args {
                b.borrow_mut().casks.push(s.to_string_lossy());
            }
            Ok(Value::Nil)
        });
    }
    {
        ...
        rt.register_fn("mas", move |args| {
            if let [Value::Str(name), Value::Int(id)] = args {
                b.borrow_mut().mas_apps.push((name.to_string_lossy(), *id));
            }
            Ok(Value::Nil)
        });
    }
