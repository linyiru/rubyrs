...
fn run_active_builder_style() {
    ...
    rt.register_fn("host_register_server", move |args| {
        if let [Value::Str(name), Value::Str(itype), Value::Int(port)] = args {
            topo_for_server.borrow_mut().servers.push(ServerConfig {
                ...
                instance_type: itype.to_string_lossy(),
                port: *port,
            });
        }
        Ok(Value::Nil)
    ...
    rt.register_fn("host_register_route", move |args| {
        if let [Value::Str(lb_name), Value::Str(path), Value::Str(target)] = args {
            ...
            if let Some(lb) = topo.load_balancers.iter_mut().find(|l| l.name == lb_name.to_string_lossy()) {
                lb.routes.push((path.to_string_lossy(), target.to_string_lossy()));
            }
        }
        Ok(Value::Nil)
    });
                    ...
                    for (sk, sv) in s_hash {
                        if let Value::Sym(ssym) = sk {
                            match rt.resolve_sym(ssym) {
                                "name" => if let Value::Str(s) = sv { server.name = s.to_string_lossy(); },
                                "type" => if let Value::Str(s) = sv { server.instance_type = s.to_string_lossy(); },
                    ...
                    for (lk, lv) in lb_hash {
                        if let Value::Sym(lsym) = lk {
                            match rt.resolve_sym(lsym) {
                                "name" => if let Value::Str(s) = lv { lb.name = s.to_string_lossy(); },
                                "routes" => {
                                        ...
                                        for (rk, rv) in r_hash {
                                            if let Value::Sym(rsym) = rk {
                                                match rt.resolve_sym(rsym) {
                                                    "path" => if let Value::Str(s) = rv { path = s.to_string_lossy(); },
                                                    "target" => if let Value::Str(s) = rv { target = s.to_string_lossy(); },
                                                    _ => {}
                                                }
                                            }
                                        }
