...
CARGO_PROFILE_FLAG=""
if [ "$PROFILE" = "release" ]; then CARGO_PROFILE_FLAG="--release"; fi
...
OUT="$SCRIPT_DIR/mini_json.$EXT"
cc "${LDFLAGS[@]}" -I "$WORKSPACE_ROOT/crates/rubyrs-cext/include" "$SCRIPT_DIR/mini_json.c" -o "$OUT"
echo "built $OUT"
