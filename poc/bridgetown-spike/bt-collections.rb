...
SITE = "/tmp/bt-coll-site"
require "fileutils"
FileUtils.rm_rf(SITE)
FileUtils.mkdir_p("#{SITE}/src/_layouts")
FileUtils.mkdir_p("#{SITE}/src/_posts")

File.write("#{SITE}/src/_layouts/default.html",
  "---\n---\n<!doctype html><title>{{ page.title }}</title><body>{{ content }}</body>")

# Two posts in a collection
File.write("#{SITE}/src/_posts/2024-01-01-hello.md",
  "---\nlayout: default\ntitle: Hello World\ndate: 2024-01-01\ntags: [intro, news]\n---\nFirst post body.\n")
File.write("#{SITE}/src/_posts/2024-02-15-update.md",
  "---\nlayout: default\ntitle: Big Update\ndate: 2024-02-15\ntags: [news]\n---\nSecond post body.\n")

# Index with a Liquid for-loop over the posts collection + filters
File.write("#{SITE}/src/index.md", <<~MD)
  ---
  layout: default
  title: Blog Index
  ---
  # Posts ({{ collections.posts.resources.size }})

  {% for post in collections.posts.resources %}
  - [{{ post.data.title }}]({{ post.relative_url }}) — {{ post.data.date | date: "%Y-%m" }} — {{ post.data.tags | join: ", " }}
  {% endfor %}

  Upper: {{ "hi" | upcase }} | Where: {{ collections.posts.resources | where: "title", "Big Update" | map: "title" | join: "" }}
MD

cfg = Bridgetown.configuration(
  root_dir: SITE, source: "#{SITE}/src", destination: "#{SITE}/output", quiet: true,
  template_engine: "liquid",
  collections: { "posts" => { "output" => true, "permalink" => "/blog/:name/" } }
)
puts "== collections + liquid build =="
begin
  site = Bridgetown::Site.new(cfg)
  site.process
  out = Dir.glob("#{SITE}/output/**/*").select { |f| File.file?(f) }.sort
  puts "OK: wrote #{out.size} file(s): #{out.map { |f| f.sub("#{SITE}/output", "") }.join(", ")}"
  idx = "#{SITE}/output/index.html"
  puts "--- index.html ---\n#{File.read(idx)}" if File.exist?(idx)
rescue Exception => e
